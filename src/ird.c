...
u8 IRD_match(char *titleID, char *IRD_PATH)
{
	...
	fread(&magic, 1, sizeof(u32), in);
	if (magic != 0x33495244) {
		print_load((char *)"Error : IRDU magic...");
		fclose(in);
		//Delete(IRDU_PATH);
		return  NO;
	}
...
int Check_IRDMD5(char *IRD_PATH, char *GAME_PATH)
{
	...
	if (sizeof(s.st_size) != 8) {
		print_load((char *)"Error!: stat st_size must be a 64 bit number!  (size %i)\n", sizeof(s.st_size));
		get_input_char();
		return -1;
	}
	...
	if (!fp) {
		print_load((char *)"Error!: Cannot open ISO file\n");
		get_input_char();
		return -1;
	}

	if (fseek(fp, 0x8800, SEEK_SET)<0) {
		print_load((char *)"Error!: in sect_descriptor fseek\n\n");
		goto err;
	}

	if (fread((void *)&sect_descriptor, 1, 2048, fp) != 2048) {
		print_load((char *)"Error!: reading sect_descriptor\n\n");
		goto err;
	}

	if (!(sect_descriptor.type[0] == 2 && !strncmp((const char *)&sect_descriptor.id[0], "CD001", 5))) {
		print_load((char *)"Error!: UTF16 descriptor not found");
		goto err;
	}
	...
	if (fseek(fp, lba0 * 2048, SEEK_SET)<0) {
		print_load((char *)"Error!: in path_table fseek\n\n");
		goto err;
	}
	...
	if (!directory_iso2) {
		print_load((char *)"Error!: in directory_is malloc()\n\n");
		goto err;
	}
	...
	if (!sectors) {
		print_load((char *)"Error!: in sectors malloc()\n\n");
		goto err;
	}
	...
	if (!sectors2) {
		print_load((char *)"Error!: in sectors2 malloc()\n\n");
		goto err;
	}
	...
	if (!sectors3) {
		print_load((char *)"Error!: in sectors3 malloc()\n\n");
		goto err;
	}

	if (fread((void *)sectors, 1, size0, fp) != size0) {
		print_load((char *)"Error!: reading path_table\n\n");
		goto err;
	}
	...
	while (p < size0) {
		...
		while (1) {
			...
			if (fseek(fp, ((u64)lba) * 2048ULL, SEEK_SET)<0) {
				print_load((char *)"Error!: in directory_record fseek\n\n");
				goto err;
			}
			...
			if (fread((void *)sectors2, 1, 2048, fp) != 2048) {
				print_load((char *)"Error!: reading directory_record sector\n\n");
				goto err;
			}
			...
			if (q2 == 0) {
				...
				if ((int)idr->name_len[0] == 1 && idr->name[0] == 0 && (u64)lba == (u64)(isonum_731((unsigned char *)idr->extent)) && idr->flags[0] == 0x2) {
					...
					print_load((char *)"Error!: Bad first directory record! (LBA %i)\n\n", lba);
					goto err;
				}
			}
				...
				if ((int)idr->name_len[0] > 1 && idr->flags[0] != 0x2 &&
					...
					if (file_aux[0]) {
						if (strcmp(string, file_aux)) {
							print_load((char *)"Error!: in batch file %s\n", file_aux);
							goto err;
						}
